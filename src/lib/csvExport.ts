...
export function exportAnalyticsReport(batches: MessageBatch[]) {
  ...
  downloadCsv(`analytics-report-${format(new Date(), 'yyyy-MM-dd')}.csv`, csv);
}

export function exportBatchDetails(batch: MessageBatch, logs: MessageLog[]) {
  ...
  downloadCsv(`batch-${safeName}-${format(new Date(), 'yyyy-MM-dd')}.csv`, csv);
}
