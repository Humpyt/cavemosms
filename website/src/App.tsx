...
import { ArrowRight, BarChart3, MessageSquareShare, ShieldCheck, Users2, Smartphone } from 'lucide-react';
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-background font-sans text-foreground selection:bg-primary/20">
      ...
      <div className="fixed top-6 left-0 right-0 z-50 flex justify-center px-4">
        <nav className="flex items-center justify-between px-6 py-3 rounded-full backdrop-blur-xl bg-[#0a0a0a]/80 border border-white/10 text-white shadow-2xl w-full max-w-4xl">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="Camo SMS Logo" className="h-6 w-6 object-contain" />
            <span className="font-display font-bold text-lg tracking-tight">Camo SMS</span>
          </div>
      ...
      <main>
        ...
        <section className="relative overflow-hidden pt-32 pb-20 lg:pt-48 lg:pb-32 bg-black text-white min-h-[90vh] flex flex-col justify-center">
          ...
          <div className="container mx-auto px-6 relative z-10 flex flex-col items-center text-center">
            ...
            >
              <div className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/20 px-4 py-1.5 mb-8 text-xs font-semibold uppercase tracking-wider text-primary backdrop-blur-sm">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                },
                {
                  icon: ShieldCheck,
                  title: 'Privacy First',
                  description: 'No third-party APIs. Your data, contacts, and message history stay on your device.'
                }
              ].map((feature, idx) => (
                <motion.div 
                  key={idx}
                  ...
                  className="group rounded-[2rem] bg-white/[0.02] backdrop-blur-sm p-8 border border-white/[0.05] shadow-lg transition-all duration-300 hover:-translate-y-1 hover:bg-white/[0.04] hover:border-primary/30 hover:shadow-[0_0_40px_rgba(123,235,120,0.1)] relative overflow-hidden"
                >
                  <div className="absolute top-0 right-0 w-32 h-32 bg-primary/10 rounded-bl-[100px] -mr-10 -mt-10 transition-transform duration-500 group-hover:scale-150 group-hover:bg-primary/20 blur-2xl z-0" />
                  
                  <div className="relative z-10">
                    <div className="h-14 w-14 rounded-2xl bg-white/[0.05] border border-white/10 flex items-center justify-center mb-6 transition-colors group-hover:border-primary/50 group-hover:bg-primary/10">
                    ...
                    <p className="text-white/60 text-base leading-relaxed group-hover:text-white/80 transition-colors">{feature.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      {/* Footer */}
      <footer className="bg-foreground py-8 border-t border-white/10 text-center">
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <img src="/logo.png" alt="Camo SMS Logo" className="h-6 w-6 object-contain grayscale opacity-70" />
            <span className="font-display font-bold text-background opacity-70">Camo SMS</span>
          </div>
          <p className="text-sm text-muted/50 font-medium">
            © {new Date().getFullYear()} Camo SMS. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
